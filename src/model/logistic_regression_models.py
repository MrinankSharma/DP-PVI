...
def compute_KL_qp(q_mean, q_var, p_mean, p_var):
    k = q_mean.shape[0]
    p_inv = np.inverse(p_var)
    m1_m2 = p_mean - q_mean
    KL = 0.5 * (np.trace(torch.mm(p_inv, q_var)) + np.dot(m1_m2, np.dot(p_inv, m1_m2)) - k + np.log(
        np.det(p_var)) - np.log(torch.det(q_var)))
    return KL
...
class LogisticRegressionTorchModule(nn.Module):
    ...
    def set_prior_parameters_from_numpy(self, prior_parameters_numpy):
        if prior_parameters_numpy is not None:
            self.prior_mu = torch.tensor(prior_parameters_numpy["w_mu"])
            self.prior_log_var = torch.tensor(prior_parameters_numpy["w_log_var"])
    ...
    def forward(self, X, N_samples, parameters=None):
        """
    ...
    def compute_ELBO_loss_per_point(self, Y_pred, Y_true, parameters=None):
        ...
        # compute the KL term
        KL = compute_KL_qp(self.w_mu, torch.diag(torch.exp(self.w_log_var)), self.prior_mu,
...
class MeanFieldMultiDimensionalLogisticRegression(Model):
    ...
    def __init__(self, parameters, hyperparameters):
        ...
        self.torch_module = LogisticRegressionTorchModule(parameters, hyperparameters)
        self.N_samples = hyperparameters["N_samples"]
        self.N_steps = hyperparameters["N_steps"]
        self.prediction_integration_limit = hyperparameters["prediction_integration_limit"]
        self.n_in = hyperparameters['n_in']

        base_optimizer = hyperparameters['base_optimizer_class'](self.torch_module.parameters(),
                                                                 **hyperparameters['base_optimizer_parameters'])

        self.wrapped_optimizer = hyperparameters['wrapped_optimizer_class'](optimizer=base_optimizer,
                                                                            **hyperparameters[
    ...
    def set_parameters(self, nat_parameters):
        parameters = nat_params_to_params(nat_parameters)
        if parameters is not None:
    ...
    def predict(self, x, integration_limit=500, parameters=None, hyperparameters=None):
        ...
        return self.torch_module.predict(x, integration_limit, parameters, hyperparameters).numpy()
    ...
    def fit(self, data, t_i, parameters=None, hyperparameters=None):
        ...
        cav_nat_params = B.subtract_params(self.parameters, t_i)
        # numpy dict for the effective prior
        effective_prior = nat_params_to_params_dict(cav_nat_params)
        self.torch_module.set_prior_parameters_from_numpy(effective_prior)

        # lets just do this for the time being
        for i in range(self.N_steps):
            self.wrapped_optimizer.fit_batch(X, y_true)
